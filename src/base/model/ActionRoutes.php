...
class ActionRoutes extends Model
{
     ...
     * @return string | null
     */
    public function getRouteName(string $controllerClass,string $actionName) : string | null
    {
        if (!class_exists($controllerClass)) {
            return null;
        }
        ...
        $controllerId = Inflector::camel2id(substr($className, 0, -10), '-', true);
        echo $class->getNamespaceName();die;
        if (!empty($dir)) {
            $controllerId = $dir . '/' . $controllerId;
        }
            $iterator = new \RecursiveIteratorIterator(
